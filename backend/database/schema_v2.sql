...
-- IMPORTANTE: Execute este script APÓS fazer backup dos dados!
-- Este script REMOVE as tabelas antigas e cria novas estruturas

-- ============================================
-- LIMPEZA (use com cuidado!)
-- ============================================

-- Remover view primeiro (depende da tabela)
DROP VIEW IF EXISTS daily_hydration_progress CASCADE;

-- Remover tabelas antigas (comentar se quiser manter dados)
...
-- Tabela de Lembretes Unificada (NOVA)
CREATE TABLE IF NOT EXISTS reminders (
  ...
  days_of_week INTEGER[], -- [0,1,2,3,4,5,6] onde 0=Domingo
  time TIME NOT NULL,
  
  -- Campos específicos para água
  water_start_time TIME, -- horário de início dos lembretes
  water_end_time TIME,   -- horário de fim dos lembretes
  water_interval_minutes INTEGER DEFAULT 60, -- intervalo entre lembretes
  water_amount_ml INTEGER DEFAULT 200, -- quantidade padrão por lembrete
...
-- Tabela de Métricas Corporais (Evolução)
CREATE TABLE IF NOT EXISTS body_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  weight DECIMAL(5, 2) NOT NULL, -- peso em kg (ex: 75.50)
  body_fat_percentage DECIMAL(4, 2), -- % gordura corporal (ex: 15.50)
  notes TEXT,
...
-- Função para limpar todos os dados de um usuário
...
-- Função para deletar conta permanentemente (incluindo o usuário)
CREATE OR REPLACE FUNCTION delete_account_permanently(p_user_id UUID)
RETURNS void AS $$
BEGIN
  ...
  -- Deletar o próprio usuário (IRREVERSÍVEL!)
...
COMMENT ON TABLE users IS 'Usuários do aplicativo';
COMMENT ON TABLE workouts IS 'Treinos criados pelos usuários';
COMMENT ON TABLE exercises IS 'Exercícios de cada treino';
COMMENT ON TABLE reminders IS 'Lembretes unificados (água, treino, customizados)';
COMMENT ON TABLE workout_history IS 'Histórico de treinos realizados';
COMMENT ON TABLE hydration_history IS 'Histórico de consumo de água';
COMMENT ON TABLE body_metrics IS 'Métricas corporais (peso e % gordura) para acompanhamento de evolução';

COMMENT ON COLUMN reminders.type IS 'Tipo: water, workout, custom';
COMMENT ON COLUMN reminders.recurrence IS 'Recorrência: daily, weekly, weekdays, weekends, custom';
COMMENT ON COLUMN reminders.days_of_week IS 'Array de dias da semana (0=Domingo até 6=Sábado)';
COMMENT ON COLUMN hydration_history.source IS 'Origem: manual (usuário), reminder (lembrete automático), auto (sistema)';
COMMENT ON COLUMN body_metrics.weight IS 'Peso em quilogramas';
COMMENT ON COLUMN body_metrics.body_fat_percentage IS 'Percentual de gordura corporal';
...
-- Os lembretes serão criados pelos usuários
